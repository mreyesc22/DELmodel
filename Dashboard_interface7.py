...
def save_feedback(tab_name, message_dict):
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    filename = "stakeholder_feedback.csv"
    fieldnames = ["timestamp", "tab", *message_dict.keys()]
    ...
    with open(filename, mode="a", newline='', encoding="utf-8") as csvfile:
        ...
        row = {"timestamp": timestamp, "tab": tab_name, **message_dict}
        writer.writerow(row)
...
with tab1:
    ...
    #t.image("Location.png", caption="Reference: Bottom depression location - Haringvliet estuary", use_container_width=True)
...
with tab4:
    ...
    with col1:
        ...
        dynamic_pits = st.multiselect("Dynamic Equilibrium Location (DEL)",
    ...
    with st.form(key="feedback_tab4"):  # Assuming tab index 3
        ...
        q1 = st.text_input("1. How many inlet volume options (gate openings) are shown in this scenario?")
        q2 = st.text_input("2. Which inlet volume option shows the **highest relative uncertainty** according to the boxplot?")
        q3 = st.text_area("3. Approximately how much time did you need to review and understand all the scenarios and dashboard windows?")
